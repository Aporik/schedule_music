...
<template>
  <div class="page youtube-page">
    ...
    <AppModal :open="globalStatsOpen" title="전체 우타와꾸 통계" description="모든 VSinger 우타와꾸에서 집계한 누적 횟수입니다." @close="globalStatsOpen = false"><div class="youtube-top-tabs"><UButton :class="{ active: globalStatsMode === 'song' }" @click="globalStatsMode = 'song'">곡 제목</UButton><UButton :class="{ active: globalStatsMode === 'original_artist' }" @click="globalStatsMode = 'original_artist'">원곡 가수</UButton></div><ol class="global-stats"><li v-for="row in globalStats.data.value || []" :key="row.label"><span>{{ row.label }}</span><b>{{ row.count }}회</b></li></ol></AppModal>
